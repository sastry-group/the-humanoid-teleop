...
# class TeleVision:
#     def __init__(self, img_shape, shm_name, queue, toggle_streaming, stream_mode="webrtc", cert_file="./cert.pem", key_file="./key.pem", ngrok=True):
#         self.img_shape = (img_shape[0], 2*img_shape[1], 3)
#         self.img_height, self.img_width = img_shape[:2]

#         # Instantiate the Vuer app WITH ngrok (no certs needed)
#         self.app = Vuer(host="0.0.0.0", port=8080, queries=dict(grid=False), queue_len=3)
...
#             self.webrtc_process = Process(target=web.run_app, args=(app,), kwargs={"host": "0.0.0.0", "port": 8012, "ssl_context": ssl_context})
...
#                 src="https://f2a4-169-229-222-121.ngrok-free.app/offer",
#                 # iceServer={},
#                 key="zed",
#                 aspect=1.33334,
#                 height = 8,
...
#     tv = TeleVision(resolution_cropped, shm_name, image_queue, toggle_streaming, cert_file=cert_file, key_file=key_file, stream_mode="webrtc") 
#     while True:
#         # tv.printPoses()
#         time.sleep(0.5)
